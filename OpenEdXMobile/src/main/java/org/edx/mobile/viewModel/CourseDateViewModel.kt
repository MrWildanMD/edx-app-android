...
class CourseDateViewModel(
    ...
    var eventCreateStartTime: Long = 0L
    ...
    fun addOrUpdateEventsInCalendar(
        ...
        eventCreateStartTime = Calendar.getInstance().timeInMillis
        areEventsUpdated = updatedEvent
        _syncLoader.value = true

        viewModelScope.launch(Dispatchers.IO) {
            courseDates.value?.courseDateBlocks?.forEach { courseDateBlock ->
            ...
            eventCreateStartTime = Calendar.getInstance().timeInMillis - eventCreateStartTime
            if (eventCreateStartTime < 1000) {
                // Add 1 sec delay to dismiss the dialog to avoid flickering
                // if the event creation time is less then 1 sec
                SystemClock.sleep(1000 - eventCreateStartTime)
            }
